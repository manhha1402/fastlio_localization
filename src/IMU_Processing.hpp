...
void ImuProcess::IMU_init(
  ...
  if (gravity_align_en_) {
    ...
    if (acc0.norm() > 1e-6) {
      acc0.normalize();
      q_align_ =
          Eigen::Quaterniond::FromTwoVectors(acc0, Eigen::Vector3d::UnitZ());
      has_align_ = true;
...
void ImuProcess::UndistortPcl(
  ...
  const double imu_beg_time =
      fastlio_ros2::stamp_to_sec(v_imu.front()->header.stamp);
  const double imu_end_time =
      fastlio_ros2::stamp_to_sec(v_imu.back()->header.stamp);
  (void)imu_beg_time;
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    if (fastlio_ros2::stamp_to_sec(tail->header.stamp) < last_lidar_end_time_)
      continue;
    ...
    if (fastlio_ros2::stamp_to_sec(head->header.stamp) < last_lidar_end_time_)
      dt =
          fastlio_ros2::stamp_to_sec(tail->header.stamp) - last_lidar_end_time_;
    else
    ...
    double offs_t =
        fastlio_ros2::stamp_to_sec(tail->header.stamp) - pcl_beg_time;
